...
def test_orchestrator_crypto_binance_universe_uses_selector():
    ...
    with patch(
        "tvscreener_ext.universe.binance_crypto.build_binance_crypto_universe",
...
def test_orchestrator_crypto_binance_universe_aliases_route_correctly():
    ...
    with patch(
        "tvscreener_ext.universe.binance_crypto.build_binance_crypto_universe_tradeable_base",
        return_value=(["BINANCE:BTCUSDT"], {"count": 1, "rows": []}),
    ):
    ...
    with patch(
        "tvscreener_ext.universe.binance_crypto.build_binance_crypto_universe_tradeable_mcap_overlap",
...
def test_orchestrator_crypto_majors_minors_keywords_route_by_instrument_type():
    ...
    with patch(
        "tvscreener_ext.universe.binance_crypto.build_binance_crypto_universe_mcap_tier",
        return_value=(["BINANCE:BTCUSDT"], {"count": 1, "rows": []}),
    ):
