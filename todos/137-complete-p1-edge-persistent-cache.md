---
status: complete
priority: p1
issue_id: "137"
tags: [architecture, duckdb, persistence]
dependencies: ["134"]
---

# Implement Persistent Edge Cache for DuckDB

## Problem Statement
The current `EdgeQueryClient` uses an ephemeral `:memory:` database. Every time the CLI or an agent runs a query, the DuckDB state is lost. This prevents cross-session caching and forces redundant data registration.

## Findings
- `EdgeQueryClient.__init__` is hardcoded to `:memory:`.
- Database connections are not managed via context managers, risking locks.

## Proposed Solutions
1. **Persistent Default**: Default to `exports/.tvscreener_cache.duckdb`.
2. **Context Manager Support**: Add `__enter__` and `__exit__` to handle connection lifecycle safely.
3. **Auto-Provisioning**: Ensure the `exports/` directory exists before connecting.

## Recommended Action
Implement the persistent cache with a context manager interface.

## Acceptance Criteria
- [ ] `EdgeQueryClient` defaults to `exports/.tvscreener_cache.duckdb`.
- [ ] Directory `exports/` is created automatically.
- [ ] `with EdgeQueryClient() as client:` pattern works correctly.
- [ ] Data persists between different CLI invocations.

## Work Log
### 2026-03-02 - Task Created
- Part of Phase 2 architecture modernization.
