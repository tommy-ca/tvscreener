...
def _warehouse_uri() -> str:
    ...
    if s.mode == "local":
        wh = (s.local.base_dir / s.local.warehouse_dir).absolute()
        return f"file://{wh}"
    assert s.remote is not None
    return str(s.remote.warehouse)


def _catalog_uri() -> str:
    s = load_settings(None).lakehouse
    if s.mode == "local":
        db = (s.local.base_dir / s.local.catalog_db).absolute()
        return f"sqlite:////{db}"
    assert s.remote is not None
    return str(s.remote.uri)
...
def main(argv: list[str] | None = None) -> int:
    ...
    s = load_settings(None).lakehouse
    print(f"lakehouse.mode={s.mode}")
    if s.mode == "local":
        print(f"lakehouse.base_dir={s.local.base_dir}")
        print(f"lakehouse.catalog_db={s.local.catalog_db}")
        print(f"lakehouse.warehouse_dir={s.local.warehouse_dir}")
    ...
    if s.mode == "local":
        base = Path(s.local.base_dir)
        if base.exists():
            print(f"lakehouse.local_exists=1 path={base}")
    return 0
