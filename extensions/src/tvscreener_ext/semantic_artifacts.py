...
def _semantic_models_dir() -> Path:
    return _repo_root() / "semantic" / "models"
...
def semantic_opportunity_top_rows(
    *, data_params_hash: str, limit: int = 30
    ...
    if semantic_runtime() == "sidemantic":
        sidemantic_rows = sidemantic_opportunity_top_rows(
...
def semantic_opportunity_health(*, data_params_hash: str) -> dict[str, Any] | None:
    ...
    if semantic_runtime() == "sidemantic":
        sidemantic_health = sidemantic_opportunity_health(data_params_hash=data_params_hash)
        if sidemantic_health is not None:
            return sidemantic_health
...
def sidemantic_opportunity_grade_summary(
    *, data_params_hash: str, limit: int = 50
    ...
    try:
        ...
        load_from_directory(layer, _semantic_models_dir())

        query = (
            "SELECT "
            "  GRADE, DIRECTION, opportunity_count, avg_ensemble_score, avg_total_confluence, "
...
def sidemantic_opportunity_top_rows(
    *, data_params_hash: str, limit: int = 30
) -> list[dict[str, Any]] | None:
    """Compute Top Rows via Sidemantic, if enabled."""
    ...
    try:
        ...
        query = (
            "SELECT "
            "  PAIR, Name, Price, RVOL, Volume, ENSEMBLE_SCORE, GRADE, DIRECTION, GRID_ALIGNED, "
            "  GRID_TOTAL, CONFLUENCE_LEVEL, TOTAL_CONFLUENCE, TF_CONFLUENCE, TREND_SCORE, MA_SCORE, "
            "  OSC_SCORE, ROC_SCORE, ROC_AVG, ROC_15, ROC_60, ROC_240, TREND_DIR, MA_DIR, OSC_DIR, "
            "  ROC_DIR, RATING_SCORE, fetched_at_utc "
            "FROM opportunity_top_rows "
            f"WHERE opportunity_top_rows.params_hash = '{data_params_hash}' "
            "ORDER BY TOTAL_CONFLUENCE DESC NULLS LAST, ENSEMBLE_SCORE_ABS DESC NULLS LAST, fetched_at_utc DESC NULLS LAST "
            f"LIMIT {int(limit)}"
        )
        ...
        logger.debug("Sidemantic top-rows query failed: %s", exc)
        return None
    finally:
        with contextlib.suppress(Exception):
            import shutil
...
def sidemantic_opportunity_health(*, data_params_hash: str) -> dict[str, Any] | None:
    """Compute Health and lineage checks via Sidemantic, if enabled."""
    ...
    try:
        ...
        if not rows:
            return None
        out = dict(zip(cols, rows[0], strict=False))
        out = {k: _coerce_cell(v) for k, v in out.items()}
        out["source_table"] = "tvscreener.signals_batch"
        out["data_params_hash"] = data_params_hash
        out["dedup_partition"] = "PAIR"
        out["dedup_order"] = "TOTAL_CONFLUENCE desc, abs(ENSEMBLE_SCORE) desc, fetched_at_utc desc"
        ...
        logger.debug("Sidemantic health query failed: %s", exc)
        return None
    finally:
        with contextlib.suppress(Exception):
            import shutil
